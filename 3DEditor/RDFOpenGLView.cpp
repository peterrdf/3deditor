...
/*virtual*/ void CRDFOpenGLView::_onKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) /*override*/
{
	...
	if (!m_dragFace.IsActive() && (nChar == VK_CONTROL || nChar == VK_SHIFT) && (m_iPointedFace != -1)) {
		if (auto pModel = getController()->getModelByOwlInstance(m_pPointedInstance->getOwlModel())) {
...
void CRDFOpenGLView::DrawNormalVectors(_model* pModel)
{
	...
	if (getController()->getSelectedInstances().size() == 1) {
		auto pSelectedInstanceModel = getController()->getModelByOwlInstance(getController()->getSelectedInstances()[0]->getOwlModel());
