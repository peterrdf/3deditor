...
/*virtual*/ void CDesignTreeView::onInstancePropertyEdited(_view* pSender, _rdf_instance* pInstance, _rdf_property* pProperty) /*override*/
{
	if (pSender == this) {
		return;
	}
	...
	auto itPropertyItem = itInstance2Properties->second.find(pProperty->getRdfProperty());
	...
	switch (pPropertyItem->GetProperty()->getType()) {
		...
		case DATATYPEPROPERTY_TYPE_INTEGER:
		{
			...
			GetDatatypeProperty(pInstance->getOwlInstance(), pPropertyItem->GetProperty()->getRdfProperty(), (void**)&piValue, &iCard);
...
void CDesignTreeView::RemoveInstanceItemData(_rdf_instance* pInstance, HTREEITEM hInstance)
{
...
void CDesignTreeView::RemovePropertyItemData(_rdf_instance* pInstance, _rdf_property* pProperty, HTREEITEM hProperty)
{
	ASSERT(pInstance != nullptr);
...
void CDesignTreeView::AddProperties(HTREEITEM hParent, _rdf_instance* pInstance)
{
	...
	while (iPropertyInstance != 0) {
		...
		if (itInstance2Properties == m_mapInstance2Properties.end()) {
			...
			mapItemProperties[pProperty->getRdfProperty()] = pPropertyItem;
			...
			map<int64_t, CRDFPropertyItem*>::iterator itPropertyItem = itInstance2Properties->second.find(pProperty->getRdfProperty());
			if (itPropertyItem == itInstance2Properties->second.end()) {
				...
				itInstance2Properties->second[pProperty->getRdfProperty()] = pPropertyItem;
